...
libplp_uart_read_string:
# Reads string from uart and places it in an array
# Values are in ASCII
# Starting memory address must be in $a0
# Stores null at end of array
	move $t9, $ra # return address
	read:
	jal libplp_uart_read #uart lib
	nop
	sw $v0, 0($a0) #store to memory
	addiu $a0, $a0, -4 #memory position
	
	#check if there is more data
	lui $t0, 0xf000 #uart
	lw $t1, 4($t0) #read status of uart
	li $t0, 3 
	#if uart has no more data
	bne $t0,$t1, return 
	nop
	j read
	nop
	return:
	sw $0, 0($a0) #add null to end of array
	jr $t9
	nop
